                    ...
                    if (IsBattlerAlive(i) && !IsBattlerAlly(i, battler))
                    {
                        for (j = 0; j < MAX_MON_MOVES; j++)
                        {
                            ...
                            if (GetTypeModifier(moveType, types[0]) >= UQ_4_12(2.0)
                             ...
                             || (GetGenConfig(GEN_CONFIG_ANTICIPATION_HIDDEN_POWER) >= GEN_6 && moveEffect == EFFECT_HIDDEN_POWER && GetTypeModifier(CheckDynamicMoveType(GetBattlerMon(i), move, i, MON_IN_BATTLE), types[0]) >= UQ_4_12(2.0))
